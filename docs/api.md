...
For server-side reuse, `storage.command(command)` performs validation, transactional persistence, idempotency, and change notification. MCP tools use this entry point when enabled. `applyCommand` in the shared domain package is pure and performs no I/O.
...
`reward.save` accepts optional `currencyExchange` (boolean), `exchangeCurrency` (currency ID), `exchangeValue` (positive integer), and `oncePerDay` (boolean). Exchange currency must differ from the cost currency. Redemption atomically debits the cost and credits the payout, with both entries sharing the command ID in the ledger. `oncePerDay` limits each child to one redemption of that reward on the server’s current calendar day; a supplied different day is rejected. Retrying the same command ID does not redeem again.

Snapshots include cumulative `exchangeEarned[currency][kidId]` amounts and `rewardRedemptions[day:rewardId:kidId]` counts. Daily snapshots include only that day’s redemption counts; exchange earnings are already included in `balanceCarry`, so their `exchangeEarned` is empty. Older rewards default to neither option enabled.
