...
- A Currency settings page with built-in credits that can be turned on and renamed: Star and Gold Star (on by default), plus Coin, Dollar, Hours, Custom 1, and Custom 2. Built-in marks use theme-specific SVG silhouettes; Gold Star shimmers except in high contrast mode or when reduced motion is enabled. Custom 1 and Custom 2 pick an emoji. Internal identifiers do not change.
...
- High contrast mode uses outlined Gold Stars to distinguish them from solid regular Stars in both light and dark appearances.

- Child balances now sit below the child’s name and wrap without separating icons from amounts. Custom currency emoji pickers now open over Settings instead of inside a hidden task dialog.

- Redrew built-in currency icons for all seven themes with solid silhouettes, cutout banknote and clock details, Lego stud coins, consistent sizing, and colors that adapt to light and dark appearances. Theme changes now select their matching SVG set.
