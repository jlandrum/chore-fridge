...
const currency = { enum: ["star", "gold", "coin", "dollar", "hours", "custom1", "custom2"] };
const pin = { type: "string", pattern: "^(|[0-9]{4})$" };
const day = { type: "string", pattern: "^[0-9]{4}-[0-9]{2}-[0-9]{2}$" };
const object = (properties, required = Object.keys(properties)) => ({ type: "object", properties, required, additionalProperties: false });
const currencySetting = object({ id: currency, name: { type: "string", minLength: 1, maxLength: 40 }, enabled: { type: "boolean" } });
...
export const payloadSchemas = {
  ...
  'pin.set':object({pin}),
  'settings.update':{...object({familyName:text,requireParentModeForCompletion:{type:'boolean'},requireParentModeForRedemptions:{type:'boolean'},mcpEnabled:{type:'boolean'},sayings:{type:'array',maxItems:200,items:{type:'string',minLength:1,maxLength:300}},currencies:{type:'array',minItems:7,maxItems:7,items:currencySetting}},[]),minProperties:1},
...
export const stateSchema = {
  ...
  properties:{
    version:{type:'integer',const:1}, requireParentModeForCompletion:{type:'boolean'},requireParentModeForRedemptions:{type:'boolean'},mcpEnabled:{type:'boolean'}, familyName:{type:'string'},pin:{type:'string'},setupDone:{type:'boolean'},sayings:{type:'array',items:{type:'string'}},
