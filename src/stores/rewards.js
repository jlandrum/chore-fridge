...
export function saveReward(reward) {
  ...
  commit(() => $rewards.set(upsert($rewards.get(), next)));
}

export function removeReward(id) {
  commit(() => $rewards.set($rewards.get().filter((reward) => reward.id !== id)));
}
...
export function redeemReward(rewardId, kidId) {
  ...
  commit(() => spending.set({ ...spending.get(), [kidId]: (spending.get()[kidId] || 0) + reward.cost }));
